...
let priceCodeMap = null;              // Map of normalized filter code -> unit price
...
async function loadFormDefs() {
    ...
    // 1. Load primary prices
    (globalData.prices || []).forEach(p => {
        const rawCode = p.SupplierFilterCode || '';
        const price = p.UnitPriceLKR || p.TotalPriceLKR || 0;
        if (!price) return;
        
        map.set(rawCode.toUpperCase().trim(), price);
        map.set(norm(rawCode), price);
        
        const extracted = extractCodes(rawCode);
        extracted.forEach(c => map.set(c.toUpperCase(), price));
    });
    
    // 2. Link filter cross-references from the catalog
    (globalData.filters || []).forEach(f => {
        ...
        let price = null;
        if (normOem && map.has(normOem)) price = map.get(normOem);
        else if (normHifi && map.has(normHifi)) price = map.get(normHifi);
        else {
            // Check if any code in CrossReferences matches an existing priced code
        ...
        if (price) {
            // Associate this filter's OEM, HIFI, and equivalents with the price
            if (oem) {
                map.set(oem.toUpperCase().trim(), price);
                map.set(normOem, price);
            }
            if (hifi) {
                map.set(hifi.toUpperCase().trim(), price);
                map.set(normHifi, price);
            }
            const crossCodes = extractCodes(cross);
            crossCodes.forEach(c => {
                ...
                if (!map.has(normC)) map.set(normC, price);
            });
        }
    });
...
function lookupFilterPrice(no) {
    ...
    if (priceCodeMap.has(key)) return priceCodeMap.get(key);
    
    const normKey = key.replace(/[^A-Z0-9]/g, '');
    if (priceCodeMap.has(normKey)) return priceCodeMap.get(normKey);
    
    // Substring fallback checks
    for (const [code, price] of priceCodeMap.entries()) {
        if (code.includes(key) || key.includes(code)) return price;
        if (normKey && code.includes(normKey)) return price;
    }
    return null;
}
function lookupOilPrice(type) {
    if (!type || !globalData.oilPrices) return null;
    const key = type.toUpperCase().trim();
    let hit = globalData.oilPrices.find(x => (x.OilTypeCode || '').toUpperCase() === key);
    if (!hit) hit = globalData.oilPrices.find(x => (x.OilTypeCode || '').toUpperCase().includes(key) || key.includes((x.OilTypeCode || '').toUpperCase()));
    return hit ? hit.UnitPriceLKR : null;
}
...
function renderFormMatrices() {
    ...
    oc.innerHTML = oilDefs.map((o, i) => `
        <div class="matrix-row">
            ...
            <div class="matrix-cell"><input type="text" id="oil-type-${i}" list="oilTypes" oninput="oilTypeChanged(${i})"></div>
            <div class="matrix-cell"><input type="text" id="oil-cv-${i}" class="upper" placeholder="C/V"></div>
            <div class="matrix-cell"><input type="number" id="oil-l-${i}" step="0.1" data-unit="0" oninput="oilQtyChanged(${i})"></div>
    ...
    fc.innerHTML = filterCatDefs.map((f, i) => `
        <div class="matrix-row">
            ...
            <div class="matrix-cell"><input type="text" id="flt-no-${i}" list="filterCodes" data-default="${f.UnitPrice || 0}" data-unit="0" oninput="filterNoChanged(${i})"></div>
            <div class="matrix-cell"><input type="number" id="flt-qty-${i}" min="1" value="1" oninput="filterQtyChanged(${i})"></div>
    ...
    if (!document.getElementById('oilTypes')) {
        const dl = document.createElement('datalist');
        dl.id = 'oilTypes';
        dl.innerHTML = (globalData.oilPrices || []).slice(0, 500)
            .map(p => `<option value="${esc(p.OilTypeCode)}">`).join('');
        document.body.appendChild(dl);
    }
...
function oilTypeChanged(i) {
    const type = document.getElementById(`oil-type-${i}`).value;
    const qEl = document.getElementById(`oil-l-${i}`);
    const priceEl = document.getElementById(`oil-price-${i}`);
    const found = lookupOilPrice(type);
    if (found != null && found > 0) {
        qEl.dataset.unit = found;
    } else {
        qEl.dataset.unit = "0";
    }
    const qty = parseFloat(qEl.value) || 0;
    const unit = parseFloat(qEl.dataset.unit) || 0;
    if (unit > 0 && qty > 0) priceEl.value = Math.round(qty * unit * 100) / 100;
    recalcTotals();
}
...
function filterNoChanged(i) {
    const noEl = document.getElementById(`flt-no-${i}`);
    const no = noEl.value;
    const priceEl = document.getElementById(`flt-price-${i}`);
    const qtyEl = document.getElementById(`flt-qty-${i}`);
    const found = lookupFilterPrice(no);
    const unit = found != null && found > 0 ? found : (parseFloat(noEl.dataset.default) || 0);
    noEl.dataset.unit = unit;
    const qty = parseFloat(qtyEl.value) || 1;
    if (unit > 0) priceEl.value = Math.round(qty * unit * 100) / 100;
    recalcTotals();
}
function filterQtyChanged(i) {
    const noEl = document.getElementById(`flt-no-${i}`);
    const priceEl = document.getElementById(`flt-price-${i}`);
    const qtyEl = document.getElementById(`flt-qty-${i}`);
    const unit = parseFloat(noEl.dataset.unit) || parseFloat(noEl.dataset.default) || 0;
    const qty = parseFloat(qtyEl.value) || 1;
    if (unit > 0) priceEl.value = Math.round(qty * unit * 100) / 100;
    recalcTotals();
}
...
function fillFormFromJob(job) {
    ...
    (job.oils || []).forEach(o => {
        ...
        if (i >= 0) {
            ...
            const unitPrice = o.Quantity ? (o.Price / o.Quantity) : lookupOilPrice(o.OilType);
            document.getElementById(`oil-l-${i}`).dataset.unit = unitPrice || 0;
        }
    });
    // filters by category
    (job.filters || []).forEach(f => {
        ...
        if (i >= 0) {
            ...
            document.getElementById(`flt-qty-${i}`).value = qty;
            const unitPrice = lookupFilterPrice(f.FilterNo) || (qty ? (f.Price / qty) : 0);
            document.getElementById(`flt-no-${i}`).dataset.unit = unitPrice || 0;
        }
    });
...
function gatherPayload() {
    ...
    filterCatDefs.forEach((f, i) => {
        ...
        const qty = parseFloat(document.getElementById(`flt-qty-${i}`).value) || 1;
        const action = document.getElementById(`flt-xe-${i}`).value;
        const price = parseFloat(document.getElementById(`flt-price-${i}`).value);
        if (no || action || price) payload.filters.push({ category: f.Name, no, quantity: qty, action, price: price || 0 });
    });
    for (let i = 1; i <= 5; i++) {
...
function renderJobCard(job) {
    ...
    <div class="job-card">
        <div class="job-header">
            <span><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 4px; display: inline-block;"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg> ${fmtDate(job.ServiceDate)} ${job.JobNo ? '· Job ' + esc(job.JobNo) : ''}</span>
