...
const PRICE_TABS = {
    ...
    oilbook: {
        route: 'oil-prices', idCol: 'PriceID', label: 'Oil Price', searchable: true,
        cols: [
            { key: 'OilTypeCode', label: 'Oil Type Code', type: 'text', w: '30%' },
            { key: 'Description', label: 'Description', type: 'text', w: '50%' },
            { key: 'UnitPriceLKR', label: 'Unit Price', type: 'number', w: '20%' }
        ],
        defaults: { OilTypeCode: '', Description: '', UnitPriceLKR: 0 }
    },
    filtercats: {
...
async function reloadCatalogIfPrices() {
    if (currentPriceTab !== 'filterbook' && currentPriceTab !== 'oilbook') { oilDefs = filterCatDefs = null; return; }
    try { const db = await api('/api/catalog'); globalData.prices = db.prices || []; globalData.oilPrices = db.oilPrices || []; priceCodeMap = null; oilDefs = filterCatDefs = null; } catch (e) {}
}
